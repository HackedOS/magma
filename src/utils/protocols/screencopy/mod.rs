...
use crate::utils::protocols::screencopy::frame::{Screencopy, ScreencopyFrameState};
    ...
    D: 'static,
{
    fn request(
        ...
        let (frame, overlay_cursor, rect, output) = match request {
            ...
            Request::CaptureOutputRegion { frame, overlay_cursor, x, y, width, height, output } => {
                let rect = Rectangle::from_loc_and_size((x, y), (width, height));

                // Translate logical rect to physical framebuffer coordinates.
                let output = state.output(&output);
                let output_transform = output.current_transform();
                let rotated_rect = output_transform.transform_rect_in(rect, &output.current_mode().unwrap().size);
        ...
        let frame = data_init.init(frame, ScreencopyFrameState { overlay_cursor, rect, output });
...
macro_rules! delegate_screencopy_manager {
    ($(@<$( $lt:tt $( : $clt:tt $(+ $dlt:tt )* )? ),+>)? $ty: ty) => {
        ...
        ] => $crate::utils::protocols::screencopy::ScreencopyManagerState);
        ...
        smithay::reexports::wayland_server::delegate_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            smithay::reexports::wayland_protocols_wlr::screencopy::v1::server::zwlr_screencopy_frame_v1::ZwlrScreencopyFrameV1: $crate::utils::protocols::screencopy::frame::ScreencopyFrameState
        ] => $crate::utils::protocols::screencopy::ScreencopyManagerState);
    };
}
